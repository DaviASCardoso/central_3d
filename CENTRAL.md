...
Complementam a stack `trimesh` para checagem de malha, `lib3mf` para escrita de 3MF (ver a emenda da seção 9 sobre por que a chamada é direta e não pela classe `Mesher` do build123d), `watchdog` para recarregamento a quente, `pydantic` para validação de parâmetros e serialização de presets, `sqlite3` da biblioteca padrão para o catálogo, e `pytest` para os testes. Gerenciamento de ambiente com `uv`.
...
> **Verificado em 2026-07-28.** Esses desvios lineares são **absolutos, em milímetros**, e por isso a chamada obrigatoriamente passa `isRelative=False` ao `BRepMesh_IncrementalMesh`. Com o padrão `isRelative=True` o valor vira fração do tamanho da aresta e os números acima perdem o sentido — num cilindro de raio 20 mm, 0,08, 0,015 e 0,005 produzem a mesma malha de 248 triângulos. Verificado também que o OCCT emite a triangulação por face, com vértices duplicados nas costuras: sem soldagem dos vértices depois de tesselar, a malha nunca é estanque e o portão de qualidade reprovaria tudo. Detalhes em `docs/NOTAS_VERIFICACAO.md`.
...
A malha de exportação é carregada no `trimesh` e verificada quanto a estanqueidade com `is_watertight`, volume estritamente positivo, ausência de faces degeneradas e duplicadas, e coerência de normais com `is_winding_consistent`. Falha em estanqueidade é bloqueante e impede a exportação, porque malha não-manifold quebra no fatiador e o operador só descobriria dez minutos depois no Bambu Studio. Faces degeneradas em pequena quantidade geram aviso e uma tentativa automática de reparo, seguida de nova checagem.

> **Verificado em 2026-07-28.** O `trimesh` 4.12.2 removeu `Trimesh.remove_degenerate_faces()`. O descarte de faces degeneradas passa a ser `malha.update_faces(malha.nondegenerate_faces())`, e o reparo continua em `trimesh.repair`, com `fix_winding`, `fix_normals`, `fix_inversion` e `fill_holes`.
...
O formato padrão é **3MF**, escrito sobre o lib3mf, porque ele carrega unidades explicitamente, suporta múltiplos objetos nomeados num mesmo arquivo, e é o formato nativo do ecossistema Bambu. STL continua disponível como alternativa para compatibilidade, e STEP como exportação de engenharia para quem quiser editar a peça em outro CAD. A escolha do formato fica nas configurações com o 3MF como padrão, e a caixa de exportação permite sobrepor pontualmente.

> **Verificado em 2026-07-28, e esta é uma emenda ao que o documento dizia antes.** A escrita do 3MF chama o `lib3mf` **diretamente**, e não a classe `Mesher` do build123d. Razão: `Mesher.add_shape` remalha o sólido incondicionalmente — faz `deepcopy` da forma e roda `BRepMesh_IncrementalMesh` com desvio **relativo** —, de modo que a malha gravada no arquivo nunca é a malha que o portão de qualidade da seção 8 aprovou. Isso esvaziaria o portão: aprova-se uma peça e entrega-se outra. Chamando o lib3mf direto, o arquivo recebe exatamente os vértices e triângulos validados. A dependência é a mesma; muda apenas quem a invoca.

Corpos múltiplos vão como objetos separados dentro do mesmo 3MF por padrão, preservando nomes e cores. Uma opção permite exportar cada corpo como arquivo próprio, útil quando tampa e base vão para impressões diferentes.

> **Verificado em 2026-07-28.** O campo `Corpo.filamento` **não é gravável** em 3MF genérico: índice de filamento e atribuição de AMS são extensão proprietária do Bambu, guardada em `Metadata/model_settings.config` dentro do pacote, e a seção 10 já põe o formato de projeto do Bambu fora de escopo. O que vai para o arquivo é o nome de cada corpo, como atributo `name` do `<object>`, e a cor, como `<basematerials>` com `displaycolor`. O campo `filamento` permanece no contrato como metadado de exibição e para uso futuro.
...
> **Situação em 2026-07-28.** Os três primeiros pontos foram verificados contra as versões instaladas e a evidência bruta está em `docs/NOTAS_VERIFICACAO.md`. Em resumo: o `Mesher` remalha e por isso a exportação passou a chamar o lib3mf direto (emenda na seção 9); `vtkmodules.qt.QVTKRenderWindowInteractor` existe na build de VTK 9.6.2 e funciona com PySide6, com o binding fixado explicitamente antes do import; e o Bambu Studio 02.07.01.62 não tem flag de abrir arquivo — o caminho vai como argumento posicional —, além de gravar um `result.json` no diretório de trabalho corrente, o que obriga a invocá-lo com `cwd` controlado. A calibragem dos desvios de tesselagem continua pendente, por depender de olhar o resultado real, e os valores atuais já foram confirmados como absolutos e produtores de malha estanque.
